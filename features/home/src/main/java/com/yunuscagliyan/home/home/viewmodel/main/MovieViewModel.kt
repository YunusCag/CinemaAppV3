...
class MovieViewModel @Inject constructor(
    ...
    private fun getMovies() {
        upComingMovies = getMovieListByType(type = MoviePagingType.UPCOMING).cachedIn(viewModelScope)
        trendingMovies = getMovieListByType(type = MoviePagingType.TRENDING).cachedIn(viewModelScope)
        popularMovies = getMovieListByType(type = MoviePagingType.POPULAR).cachedIn(viewModelScope)
        topRatedMovies = getMovieListByType(type = MoviePagingType.TOP_RATED).cachedIn(viewModelScope)
    }
